...
// Pages where navbar should be shown (only home page)
const NAVBAR_PAGES = ['/', '/about'];

export default function Layout({ children }: LayoutProps) {
  const pathname = usePathname();
  
  // Only show navbar on specific pages (home, about)
  const showNavbar = NAVBAR_PAGES.includes(pathname);

  return (
    <div className="min-h-screen flex flex-col">
      {/* Transparent Blur Navbar - Only on home/about pages */}
      {showNavbar && (
